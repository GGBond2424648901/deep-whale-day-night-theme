...
describe('standalone distribution metadata', () => {
  it('embeds the cleaned v2 day and night scene assets', () => {
    const embedScript = readText('scripts/embed-deep-whale-art.mjs')

    expect(embedScript).toContain("['DEEP_WHALE_DAY_SCENE', 'image/webp', 'deep-whale-day-scene-v2.webp']")
    expect(embedScript).toContain("['DEEP_WHALE_NIGHT_SCENE', 'image/webp', 'deep-whale-night-scene-v2.webp']")
